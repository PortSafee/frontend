...
const RegisterDeliveryPage: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 flex flex-col justify-center items-center bg-gradient-to-r from-[#002236] via-black to-[#002134] z-50">
        <img
          src={Logo.src}
          alt="Logo PortSafe"
          className="w-32 sm:w-48 md:w-64 lg:w-80 mb-4 max-w-full"
        />
        <div className="w-12 h-12 sm:w-16 sm:h-16 border-4 border-t-4 border-t-[#00C2FF] border-[#00C2FF]/30 rounded-full animate-spin mb-4"></div>
        <p className="text-white text-base sm:text-lg md:text-xl">Gerando código de entrega...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-gradient-to-r from-[#002236] via-black to-[#002134] overflow-x-hidden">
      ...
      <div className="flex flex-col justify-center items-center w-full md:w-1/2 text-center p-6">
        <img
          src={Logo.src}
          alt="Logo PortSafe"
          className="w-32 sm:w-48 md:w-64 lg:w-80 mb-2 max-w-full"
        />
      ...
      <div className="flex flex-col justify-center items-center w-full md:w-1/2 px-4 sm:px-6 md:px-10 overflow-y-auto">
        {/* Barra de progresso */}
        <div className="w-full max-w-md mr-0 md:mr-16 mb-6">
        ...
        {/* Card de registro */}
        <div className="bg-[#ffffff18] border-2 border-[#606060] mr-0 md:mr-16 rounded-2xl w-full max-w-md text-center shadow-lg overflow-hidden">
          {/* Cabeçalho do card */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-6 px-4 sm:px-8">
            {/* Entrega Padrão */}
            <div
              onClick={() => setTipoEntrega("Padrão")}
              className={`cursor-pointer p-3 sm:p-4 md:p-5 rounded-xl border w-full ${
                tipoEntrega === "Padrão"
          ...
          {(tipoEntrega === "Volumosa" || tipoEntrega === "Perecível") && (
            <>
              <div className="fixed inset-0 bg-[#2258847d] bg-opacity-50 z-50 flex items-center justify-center p-4">
                <div className="bg-gradient-to-r from-[#000000] to-[#061f34] rounded-xl p-6 w-full max-w-md text-center">
                  <h3 className="title font-marmelad text-2xl sm:text-3xl font-bold text-[#E9D9AE] mt-4 mb-4">Atenção!</h3>
                  <p className="text-gray-300 text-base sm:text-xl mb-6">Para esse tipo de encomenda é necessário se dirigir à portaria!</p>
                  <Button 
                    nome="Entendi" 
                    estilo="secundary" 
          ...
          <div className="flex justify-center px-4 sm:px-6 mb-8">
            <Button
              ...
              estilo="primary"
              className="w-full max-w-md text-white text-xs sm:text-sm md:text-base"
              clique={() => {
                setLoading(true);
                setTimeout(() => {
